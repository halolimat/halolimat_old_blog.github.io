...
As I mentioned before, each particle in the swarm represents a possible solution to the problem. And as I also mentioned in the [previous blog post][Heuristics], each particle try to improve its solution by learning from two sources: its movement in the problem space and the movement of the other particles of the swarm (through learning from the best solution found by any of the other particles).
